...
class TestReportsEndpoint:
    ...
    def test_get_reports_by_multiple_news_sites(
        ...
        filtered_reports = [
            ...
            if report.news_site.name in [news_sites[0].name, news_sites[1].name]
        ]

        response = client.get(
            f"/v4/reports/?news_site={news_sites[0].name},{news_sites[1].name}&limit=100"
        )
        assert response.status_code == 200
        ...
        assert len(data["results"]) == len(filtered_reports)
        assert all(
            report["title"] in [report.title for report in filtered_reports]
    ...
    def test_get_reports_search_summary(
        ...
        assert (
            ...
            == "Description of an report with a specific title"
        )
        assert len(data["results"]) == 1
    ...
    def test_soft_deleted(self, client: Client, reports: list[Report]):
